# Bulk Get Item Definitions Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a standalone script that bulk-exports Fabric item definitions across all workspaces using the `bulkExportDefinitions` (beta) API, decoding base64 payloads and writing each item's files to disk preserving the folder/file path structure.

**Architecture:** A new service class (`BulkExportItemDefinitionsService`) wraps the three API calls (POST export, GET status, GET result) and returns raw `Response` objects. A standalone script (`get_item_definitions.py`) drives a two-phase orchestration: Phase 1 fires `bulkExportDefinitions` for every workspace (retrying indefinitely on 429), Phase 2 polls all outstanding LROs using per-op `next_check_at` timestamps, then Phase 3 decodes and writes files. This project has no formal test suite — verification is done by running the script against a real workspace.

**Tech Stack:** Python 3.13, `requests`, `base64` (stdlib), `datetime`/`time` (stdlib), existing `ListWorkspacesService` + `AadService` + `App`/`BaseConfig`.

---

### Task 1: Service class

**Files:**
- Create: `services/bulkexportitemdefinitions.py`

- [ ] **Step 1: Create the service file**

```python
# services/bulkexportitemdefinitions.py
...
Run: `uv run python -c "from services.bulkexportitemdefinitions import BulkExportItemDefinitionsService; print('OK')"`
...
git commit -m "add BulkExportItemDefinitionsService"
```
...
**Files:**
- Create: `get_item_definitions.py`

- [ ] **Step 1: Create the script**

```python
# get_item_definitions.py
...
Run: `uv run python -c "import get_item_definitions; print('OK')"`

Expected output: `OK`

- [ ] **Step 3: Run against a single known workspace**

Edit the `if __name__ == "__main__":` block at the bottom of `get_item_definitions.py` to pass a real workspace ID:

```python
if __name__ == "__main__":
    main(workspace_id="<your-test-workspace-id>")
```

Run: `uv run python get_item_definitions.py`

Expected output (shape):
```
Requesting export for: My Workspace (<ws-id>)
  LRO started, op_id=<op-id>          ← or "Immediate result received"

Phase 1 complete: 0 immediate, 1 pending LRO(s)
Waiting 5.0s for next LRO check...
  LRO succeeded for workspace <ws-id>, fetching result...
Phase 2 complete: 1 LRO result(s) received

Writing definitions for 1 workspace(s)...
  Wrote N/N definition parts for workspace <ws-id>
...
cat data/item_definitions/<date>/<ws-id>/_index.json
```

- [ ] **Step 4: Restore `main()` call to process all workspaces**
...
```bash
git add get_item_definitions.py
git commit -m "add bulk export item definitions script"
```
