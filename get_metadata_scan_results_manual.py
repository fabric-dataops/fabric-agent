...
def get_scan_results(access_token, scan_id, scan_no, scan_count):
    ...
    if not scan_result_api_response.ok:
        ...
        datasources = api_response.get("datasourceInstances", [])
        workspaces = api_response.get("workspaces", [])
...
def main(modified_since: str = None):
    ...
    #     access_token=access_token,
    #     modified_since=modified_since,
    #     exclude_personal_workspaces=False,
    # )
    # workspaces = workspaces[:500]  # Scan the first 500 workspaces only
    # workspaces = list_workspaces(
    #     # filter="contains(name, '[Dev]') or contains(name, '[Test]')"
    # )
    
    workspaces_file_path = "data/workspaces/modified/chunks/2025_10_28/workspaces_chunk_7.json"
    # Read the workspaces from JSON file
    with open(workspaces_file_path, 'r', encoding='utf-8') as f:
